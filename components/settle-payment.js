      ...
      settlePayment: function () {
        ...
        PMS.globals.expenses.where({ group: `/group/${PMS.fn.getCurrentGroupId()}/` }).map((expense) => {
          expense.attributes.splitters.map((splitter) => {
            if (splitter.e_splitter.friend.user.username === PMS.fn.getUsername() && expense.attributes.payer.friend.user.username !== PMS.fn.getUsername() && self.model.get('currentReceiver').lender_profile === expense.attributes.payer.friend.resource_uri) {
